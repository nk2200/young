...
//	}
		...
		int cartcount = 0;
		 ...
		 System.out.println("customerid" + customerid);
		    try {
		        ...
		        pstmt.setString(2, customerid); // 매개변수 설정
	...
	public boolean plusQty(int goodsid, int goods_qty, String customerid) {
		Connection conn = null;
	
		
	    try {
	        conn = dataSource.getConnection(); // DataSource로부터 연결 가져오기
	        String sql = "UPDATE cart SET cart_qty = cart_qty + ? WHERE goodsid = ? AND customerid = ? "; // 올바른 SQL 작성
	        PreparedStatement stmt = conn.prepareStatement(sql);
	        conn.setAutoCommit(false); // Auto-commit 비활성화
